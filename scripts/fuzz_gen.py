...
def main():
    ...
    os.chdir(dir_code)
    subprocess.check_call(f'./autogen.sh && CC=clang-16 CXX=clang++-16 ./configure --enable-fuzz --with-sanitizers={args.sanitizers} && make clean && make -j {args.jobs}', shell=True)
