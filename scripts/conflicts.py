...
def update_comment(dry_run, pull, pulls_conflict):
    ...
    text += ''.join([f'\n* [#{p.number}]({p.html_url}) ({p.title.strip()} by {p.user.login})' for p in pulls_conflict])
    text += '\n\n'
...
def main():
    ...
    for slug in args.github_repos.split(","):
        ...
        pulls = return_with_pull_metadata(lambda: [p for p in github_repo.get_pulls(state='open', base=base_name)])
        ...
        with tempfile.TemporaryDirectory() as temp_git_work_tree:
            ...
            if args.update_comments:
                for i, pull_update in enumerate(pulls_mergeable):
                    print('{}/{} Checking for conflicts {} <> {} <> {} ... '.format(i, len(pulls_mergeable), base_name, pull_update.number, 'other_pulls'))
                    pulls_conflict = calc_conflicts(pulls_mergeable=pulls_mergeable, num=pull_update.number, base_branch=base_name)
            ...
            if args.pull_id:
                ...
                if not pull_merge:
                    print('{} not found in all {} open {} pulls'.format(args.pull_id, len(pulls), base_name))
                    sys.exit(-1)
                ...
                print(f'Checking for conflicts {base_name} <> {pull_merge.number} <> other_pulls ... ')
                conflicts = calc_conflicts(pulls_mergeable=pulls_mergeable, num=pull_merge.number, base_branch=base_name)
