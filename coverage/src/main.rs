...
fn gen_coverage(
    ...
    docker_exec(&format!(
        "cmake -B {} \
         -DBUILD_BENCH=ON -DWITH_ZMQ=ON -DWITH_MINIUPNPC=ON -DWITH_NATPMP=ON \
         -DWITH_BDB=ON -DWARN_INCOMPATIBLE_BDB=OFF \
...
fn calc_coverage(
    ...
    docker_exec(&format!("apt-get install -qq {}", "clang llvm ccache python3-zmq libsqlite3-dev libevent-dev libboost-dev libdb5.3++-dev libnatpmp-dev libminiupnpc-dev libzmq3-dev lcov build-essential cmake pkg-config bsdmainutils"));
