...
fn main() {
    ...
    let ts = fs::read_to_string(ts_path).expect("Unable to read file");
...
fn print_result(res: &str, msg: &str) {
    match res {
        "YES" => {
        ...
        "NO" => {}
        _ => {
            panic!("Content:\n{}\nFile was corrupt with abvove content!", res);
        }
    }
}
...
fn check(cache_dir: &Path, ts: &str, url: &str) {
    ...
    for msg in ts.split("<message>").skip(1) {
        ...
        let cache_file = cache_dir.join(hash(msg));
        match fs::read_to_string(&cache_file) {
            Ok(contents) => {
                print_result(&contents, msg);
            }
            Err(_) => {
